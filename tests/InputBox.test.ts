...
// ─────────────────────────────────────────────────────────────────────────────
// Ghost cursor filter logic (Correction 2: src/ui/input/InputBox.tsx)
//
// The keyboard handler filters out spurious key events that arrive from
// terminal mouse/scroll/focus events that @opentui/core doesn't fully parse.
// These tests exercise the pure filter predicates extracted from the handler.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Replica of the ghost-cursor filter logic from InputBox.tsx useKeyboard handler.
 * Returns true if the key event should be DISCARDED (filtered out).
 */
function shouldFilterKey(key: KeyEvent): boolean {
...
describe('InputBox ghost cursor filter', () => {
  ...
  describe('ANSI suffix single uppercase letters — should be filtered', () => {
    it('should filter key with raw = "A" and name = "A" (ANSI up suffix) without modifiers', () => {
      const key: KeyEvent = { raw: 'A', name: 'A', ctrl: false, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(true)
    })

    it('should filter key with raw = "B" and name = "B" (ANSI down suffix)', () => {
      const key: KeyEvent = { raw: 'B', name: 'B', ctrl: false, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(true)
    })

    it('should filter key with raw = "M" and name = "M" (mouse event suffix)', () => {
      const key: KeyEvent = { raw: 'M', name: 'M', ctrl: false, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(true)
    })

    it('should filter key with raw = "H" and name = "H" (home ANSI suffix)', () => {
      const key: KeyEvent = { raw: 'H', name: 'H', ctrl: false, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(true)
    })
  })
  ...
  describe('normal lowercase input — should NOT be filtered', () => {
    ...
    it('should NOT filter key with raw = "z" (normal letter)', () => {
      const key: KeyEvent = { raw: 'z', name: 'z', ctrl: false, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(false)
    })
  })
  ...
  describe('normal key events — should NOT be filtered', () => {
    it('should NOT filter key with raw = "h" and name = "h" (normal char)', () => {
      const key: KeyEvent = { raw: 'h', name: 'h' }
      expect(shouldFilterKey(key)).toBe(false)
    })

    it('should NOT filter return key', () => {
      const key: KeyEvent = { raw: '\r', name: 'return' }
      expect(shouldFilterKey(key)).toBe(false)
    })

    it('should NOT filter backspace key', () => {
      const key: KeyEvent = { raw: '\x7f', name: 'backspace' }
      expect(shouldFilterKey(key)).toBe(false)
    })

    it('should NOT filter Ctrl+C (has ctrl modifier)', () => {
      const key: KeyEvent = { raw: '\x03', name: 'c', ctrl: true }
      expect(shouldFilterKey(key)).toBe(false)
    })

    it('should NOT filter uppercase A when shift modifier is set (Shift+A = real input)', () => {
      const key: KeyEvent = { raw: 'A', name: 'A', ctrl: false, meta: false, shift: true }
      expect(shouldFilterKey(key)).toBe(false)
    })

    it('should NOT filter uppercase A when ctrl modifier is set', () => {
      const key: KeyEvent = { raw: 'A', name: 'A', ctrl: true, meta: false, shift: false }
      expect(shouldFilterKey(key)).toBe(false)
    })
  })
  ...
  describe('control characters without name — should be filtered', () => {
    it('should filter single-byte control char (charCode < 32) with no name', () => {
      ...
      const key: KeyEvent = { raw: '\x01', name: undefined }
      expect(shouldFilterKey(key)).toBe(true)
    })

    it('should filter \\x00 (null byte) with no name', () => {
      const key: KeyEvent = { raw: '\x00', name: undefined }
      expect(shouldFilterKey(key)).toBe(true)
    })
    ...
    it('should NOT filter space (charCode 32) with no name — space is valid input', () => {
      ...
      const key: KeyEvent = { raw: ' ', name: undefined }
      expect(shouldFilterKey(key)).toBe(false)
    })
  })
})
