...
- **Workflows:** \`/workflows\` opens the workflow monitor. \`/workflow run <name> [args-json]\`, \`pause|resume|stop|restart <run-id>\`, and \`save <run-id> <name>\` run or manage dynamic workflows; \`resume\` on a run that is not paused (interrupted or finished) relaunches it over its journal, reusing every completed \`agent()\` result.
...
An ad-hoc workflow starts from the \`workflow\` tool with inline \`script\`, a \`scriptPath\` inside the project or a previous run directory, or the \`name\` of a saved workflow; passing \`resumeFromRunId\` reuses the journaled result of every \`agent()\` call whose arguments did not change, so an edited script only reruns the first edited call and everything after it. A child workflow is invoked with \`workflow(name, args)\` or \`workflow({ scriptPath }, args)\`; outside Auto mode the child script must have been approved once, and its agents appear under a \`▸ name\` group in the monitor. Child workflows cannot start further child workflows. The runtime limits a workflow to 17 agents and limits concurrent agents to the configured concurrency (up to 16). Failures are recorded without silently shifting parallel results; workflow writers may request isolated Git worktrees.
