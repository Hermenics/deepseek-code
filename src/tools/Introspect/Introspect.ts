...
User-scoped hooks can run on \`SessionStart\`, \`PreToolUse\`, and \`PostToolUse\`. Pre/Post tool hooks are grouped by a tool-name matcher; a hook command receives a versioned JSON payload on standard input with the session, workspace, tool, and relevant input/result. A pre-tool hook may approve, block, or return a modified input. Hook commands, timeouts, enabled state, and stable identifiers are validated by Settings.
