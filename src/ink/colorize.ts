...
// Named ANSI colors accepted without the 'ansi:' prefix (compatibility shim
// for components that pass plain color names like "cyan", "green", etc.)
const PLAIN_ANSI_COLORS = new Set([
...
export const colorize = (
  ...
  // Normalize plain color names to 'ansi:' prefix so the switch below handles them
  if (PLAIN_ANSI_COLORS.has(color)) {
    // chalk aliases
    if (color === 'gray' || color === 'grey') color = 'blackBright'
    color = `ansi:${color}`
  }
