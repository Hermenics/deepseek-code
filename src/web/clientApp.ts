...
export const CLIENT_APP = String.raw`(() => {
  ...
  const onEvent = (event) => { switch (event.type) {
    ...
    case 'tool_pending': { setSessionState('Preparing ' + event.name + '…', true); const streaming = state.activity.find((item) => item.pending && item.active && item.name === event.name); if (streaming) { streaming.args = event.argsText; renderActivity(); persistState(); } else addActivity(event.name, event.argsText, true, '', '', true); break; }
    ...
    case 'subagent_blocked': { const blocked = state.activity.find((item) => item.taskId === event.id && item.active); if (blocked) { blocked.progress = 'BLOCKED · ' + event.reason; renderActivity(); } break; }
    case 'workflow_update': { const taskId = 'workflow:' + event.runId; const running = ['queued', 'running', 'paused'].includes(event.status); const existing = state.activity.find((item) => item.taskId === taskId); const detail = (event.phase ? event.phase + ' · ' : '') + event.usage.agents + ' agents · ' + formatTokens(event.usage.tokens); if (existing) { existing.active = running; existing.args = detail; existing.result = running ? '' : (event.error ? 'Error: ' + event.error : event.status); if (!running) existing.progress = ''; renderActivity(); } else addActivity('workflow ' + event.name, detail, running, running ? '' : event.status, taskId); break; }
    case 'workflow_log': { const item = state.activity.find((entry) => entry.taskId === 'workflow:' + event.runId && entry.active); if (item) { item.progress = event.value; renderActivity(); } break; }
