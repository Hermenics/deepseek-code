...
export const DEFAULT_RISK_RULES: RiskRule[] = [
  ...
  { id: 'shell:deploy', level: 'high', tool: 'shell', pattern: '*deploy*' },
...
export function assessRisk(
  ...
  for (const rule of sorted) {
    ...
    if (rule.pattern) {
      ...
      const existing = typeof args._existingLineCount === 'number' ? args._existingLineCount : 0
      matched = existing >= largeThreshold
